...
COINS_DATA_ENDPOINT = "https://api.coin-stats.com/v4/coins/"
...
def fetch_top_popular_coins(endpoint):
    ...
    for coin in popular_coins_data:
        name = coin['i']
        popular_coins_endpoints.update({f'{COINS_DATA_ENDPOINT}{name}': f'{name}'})
...
def fetch_cheapest_coins(endpoint):
    ...
    for c, p in cheapest5_dict.items():
        cheapest_coins_endpoints.update({f'{COINS_DATA_ENDPOINT}{c}': f'{c}'})
